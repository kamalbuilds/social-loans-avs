...
async function main() {
    const prover = new Prover('localhost:33247'); // talking with the prover
    const brevis = new Brevis('appsdkv2.brevis.network:9094'); // talking with the brevis gateway
    ...
    const proofRes = await prover.prove(proofReq);  // handles submitting proof request to our prover
